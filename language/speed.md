...
在企业级应用的设计中，动效需要在尽可能短的时间内有效的完成过渡。那么如何把握这个时间的长短呢？伦敦城市学院神经学创立者 [Davina Bristow](http://www.ucl.ac.uk/media/library/blinking) 提出过， 眨眼是人体最快的潜意识动作，而大多数人每分钟眨眼15次，眨眼平均持续100-150毫秒；所以我们将这个时间段定义成 Ant Design 动效的基本时间单位。
...
> 注：使用缓动时也需要结合元素所附于什么物体或使用场景来使用，如卡通可用弹性缓动，如 `Back`, 稳重性比较强建议使用基本的缓动函数 `Cubic`。
